...
#include <NcfMdv/Ncf2MdvTrans.hh>
...

{
  ...
  for (int i = 0; i < _ncFile->num_atts(); i++) {
    ...
    if (!strcmp(att->name(), Ncf::title)) {
      STRncopy(_mhdr.data_set_name, asString(att).c_str(), MDV_NAME_LEN);
    }
      
    if (!strcmp(att->name(), Ncf::source)) {
      STRncopy(_mhdr.data_set_source, asString(att).c_str(), MDV_NAME_LEN);
    }

    _addAttr2Str(att, Ncf::history, _dataSetInfo, "  Ncf:history: ");
    _addAttr2Str(att, Ncf::institution, _dataSetInfo, "  Ncf:institution: ");
    _addAttr2Str(att, Ncf::references, _dataSetInfo, "  Ncf:references: ");
    _addAttr2Str(att, Ncf::comment, _dataSetInfo, "  Ncf:comment: ");
...

{
  ...
  NcAtt* boundsAtt = _timeDim.var->get_att(Ncf::bounds);
  if (boundsAtt != NULL) {
...
void Ncf2MdvTrans::_setTimeInfoForVar(NcVar *var)
{
  ...
  string stdName;
  NcAtt* stdNameAtt = var->get_att(Ncf::standard_name);
  if (stdNameAtt != NULL) {
  ...
  if (name.compare(Ncf::forecast_period) == 0) {
  ...
  NcAtt* unitsAtt = var->get_att(Ncf::units);
  if (unitsAtt == NULL) {
  ...
  if (name.compare(Ncf::forecast_reference_time) == 0) {
  ...
  if (name.compare(Ncf::start_time) == 0) {
    if (unixTime != 0) {
  ...
  if (name.compare(Ncf::stop_time) == 0) {
    if (unixTime != 0) {
...
void Ncf2MdvTrans::_inspectDim(NcDim *dim, int jdim, ArrayDim &arrayDim)
{
  ...
  NcAtt *standardNameAtt = coordVar->get_att(Ncf::standard_name);
  string stdName;
  ...
  NcAtt *longNameAtt = coordVar->get_att(Ncf::long_name);
  string longName;
  if (longNameAtt != NULL) {
  ...
  if (stdName.compare(Ncf::projection_x_coordinate) == 0 ||
      longName.compare(Ncf::projection_x_coordinate) == 0 ||
      stdName.compare(Ncf::longitude) == 0) {
  ...
  if (stdName.compare(Ncf::projection_y_coordinate) == 0 ||
      longName.compare(Ncf::projection_y_coordinate) == 0 ||
      stdName.compare(Ncf::latitude) == 0) {
  ...
  NcAtt *positiveAtt = coordVar->get_att(Ncf::positive);
  if (positiveAtt != NULL) {
  ...
  
{
  ...
  double minAngRes = 5.0;
  _isRhi = false;
  for (int ii = 0; ii < (int) vol.getSweeps().size(); ii++) {
    const RadxSweep &sweep = *vol.getSweeps()[ii];
    double angRes = sweep.getAngleResDeg();
    if (angRes > 0.05 && angRes < minAngRes) {
      minAngRes = angRes;
    }
  ...
  if (_isRhi) {
    ...
    _nAngles = (int) (elevRange / minAngRes + 0.5);
    ...
    _nAngles = (int) (360.0 / minAngRes + 0.5);
...

{
  ...
  for (int isweep = 0; isweep < (int) vol.getSweeps().size(); isweep++) {
	...
	if (iangle < 0) {
      ...
      // check for missing beam
      if (prevIangle != -9999) {
	int delta = iangle - prevIangle;
	int nHalf = nAnglesPer360 / 2;
	if (delta > nHalf) {
	  delta -= nAnglesPer360;
	} else if (delta < -nHalf) {
	  delta += nAnglesPer360;
	...
	if (delta == 2) {
	  // fill in missing angle going clockwise
	  int imiss = iangle - 1;
	  if (imiss < 0) {
	    imiss += nAnglesPer360;
	  }
	  int mdvxStart = isweep * nxy + imiss * fhdr.nx;
	  memcpy(mdvData + mdvxStart, radxData + radxStart, fhdr.nx * sizeof(fl32));
	} else if (delta == -2) {
	  // fill in missing angle going counter clockwise
	  int imiss = iangle + 1;
	  if (imiss >= nAnglesPer360) {
	    imiss -= nAnglesPer360;
	  }
	  int mdvxStart = isweep * nxy + imiss * fhdr.nx;
	  memcpy(mdvData + mdvxStart, radxData + radxStart, fhdr.nx * sizeof(fl32));
