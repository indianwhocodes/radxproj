...
#include <NcfMdv/Ncf2MdvField.hh>
...

{
  ...
  NcAtt *gridMapAtt = _dataVar->get_att(Ncf::grid_mapping);
  if (gridMapAtt != NULL) {
    ...
    for (int ivar = 0; ivar < _ncFile->num_vars(); ivar++) {
      ...
      NcAtt *mappingNameAtt = mappingVar->get_att(Ncf::grid_mapping_name);
  ...
  NcAtt *projTypeAtt = _projVar->get_att(Ncf::grid_mapping_name);
  if (projTypeAtt == NULL) {
    TaStr::AddStr(_errStr, "ERROR - Ncf2MdvField::_setProjType");
  ...
  if (_projTypeStr.compare(Ncf::albers_conical_equal_area) == 0) {
    _projType = Mdvx::PROJ_ALBERS;
  } else if (_projTypeStr.compare(Ncf::azimuthal_equidistant) == 0) {
    _projType = Mdvx::PROJ_FLAT;
  } else if (_projTypeStr.compare(Ncf::lambert_azimuthal_equal_area) == 0) {
    _projType = Mdvx::PROJ_LAMBERT_AZIM;
  } else if (_projTypeStr.compare(Ncf::lambert_conformal_conic) == 0) {
    _projType = Mdvx::PROJ_LAMBERT_CONF;
  } else if (_projTypeStr.compare(Ncf::latitude_longitude) == 0) {
    _projType = Mdvx::PROJ_LATLON;
  } else if (_projTypeStr.compare(Ncf::polar_radar) == 0) {
    _projType = Mdvx::PROJ_POLAR_RADAR;
  } else if (_projTypeStr.compare(Ncf::polar_stereographic) == 0 ||
             _projTypeStr.compare("polar_sterographic") == 0) {
    _projType = Mdvx::PROJ_POLAR_STEREO;
  } else if (_projTypeStr.compare(Ncf::rotated_latitude_longitude) == 0) {
    _projType = Mdvx::PROJ_UNKNOWN;
  } else if (_projTypeStr.compare(Ncf::stereographic) == 0) {
    _projType = Mdvx::PROJ_OBLIQUE_STEREO;
  } else if (_projTypeStr.compare(Ncf::transverse_mercator) == 0) {
    _projType = Mdvx::PROJ_TRANS_MERCATOR;
  } else if (_projTypeStr.compare(Ncf::mercator) == 0) {
    _projType = Mdvx::PROJ_MERCATOR;
  } else if (_projTypeStr.compare(Ncf::vertical_perspective) == 0) {
...

{
  ...
  _setProjParam(Ncf::false_northing, falseNorthing);
  
  double falseEasting = 0.0;
  _setProjParam(Ncf::false_easting, falseEasting);
  ...
  switch (_projType) {
    ...
    case Mdvx::PROJ_ALBERS: {
      ...
      iret |= _setProjParam(Ncf::longitude_of_central_meridian,
                            origin_lon);

      double origin_lat = 0.0;
      iret |= _setProjParam(Ncf::latitude_of_projection_origin,
                            origin_lat);
      
      double lat1 = 0.0, lat2 = 0.0;
      iret |= _setProjParams(Ncf::standard_parallel, lat1, lat2);
    ...
    case Mdvx::PROJ_FLAT: {
      ...
      double origin_lon = 0.0;
      iret |= _setProjParam(Ncf::longitude_of_projection_origin,
                            origin_lon);
    ...
    case Mdvx::PROJ_POLAR_STEREO: {
      ...
      iret |= _setProjParam(Ncf::straight_vertical_longitude_from_pole,
                            tangent_lon);
      ...
      if (_setProjParam(Ncf::scale_factor_at_projection_origin,
                        central_scale) != 0) {
        double standard_parallel;
        if (_setProjParam(Ncf::standard_parallel,
                          standard_parallel) == 0) {
          central_scale =
    ...
    case Mdvx::PROJ_OBLIQUE_STEREO: {
      ...
      double scale = 1.0;
      iret |= _setProjParam(Ncf::scale_factor_at_projection_origin,
                            scale);
    ...
    case Mdvx::PROJ_TRANS_MERCATOR: {
      ...
      double origin_lon = 0.0;
      iret |= _setProjParam(Ncf::longitude_of_central_meridian, origin_lon);

      double origin_lat = 0.0;
      iret |= _setProjParam(Ncf::latitude_of_projection_origin, origin_lat);
      
      double scale = 1.0;
      iret |= _setProjParam(Ncf::scale_factor_at_central_meridian, scale);
    ...
    case Mdvx::PROJ_VERT_PERSP: {
      ...
      double origin_lon = 0.0;
      iret |= _setProjParam(Ncf::longitude_of_projection_origin, origin_lon);

      double origin_lat = 0.0;
      iret |= _setProjParam(Ncf::latitude_of_projection_origin, origin_lat);
      
      double ppt_ht_m = 0.0;
      iret |= _setProjParam(Ncf::perspective_point_height, ppt_ht_m);
  ...
  
{
  ...
  NcAtt* longNameAtt = _dataVar->get_att(Ncf::long_name);
  if (longNameAtt != NULL) {
    STRncopy(_fhdr.field_name_long, _asString(longNameAtt).c_str(), MDV_LONG_FIELD_LEN);
    delete longNameAtt;
  }

  NcAtt* unitsAtt = _dataVar->get_att(Ncf::units);
  if (unitsAtt != NULL) {
  ...
  
{
  ...
  NcAtt *attxName = axisVar->get_att(Ncf::standard_name);
  if (attxName != NULL) {
  ...
  NcAtt *attUnits = axisVar->get_att(Ncf::units);
  if (attUnits == NULL) {
    return 0;
  }
  ...
  
{
  ...
  string standardName;
  NcAtt *standardNameAtt = _zVar->get_att(Ncf::standard_name);
  ...
  string longName;
  NcAtt *longNameAtt = _zVar->get_att(Ncf::long_name);
  if (longNameAtt != NULL) {
  ...
  string units;
  NcAtt *unitsAtt = _zVar->get_att(Ncf::units);
  if (unitsAtt != NULL) {
  ...
  
{
  ...
  double scale = 1.0;
  NcAtt *scaleAtt = _dataVar->get_att(Ncf::scale_factor); 
  if (scaleAtt != NULL) {
  ...
  double offset = 0.0;
  NcAtt *offsetAtt = _dataVar->get_att(Ncf::add_offset); 
  if (offsetAtt != NULL) {
  ...
  switch (_dataType) {
    ...
    case ncByte: {
      ...
      NcAtt *fillAtt = _dataVar->get_att(Ncf::FillValue); 
      if (fillAtt != NULL) {
      ...
      ncbyte validMin = -128;
      NcAtt *minAtt = _dataVar->get_att(Ncf::valid_min); 
      if (minAtt != NULL) {
      ...
      ncbyte validMax = 127;
      NcAtt *maxAtt = _dataVar->get_att(Ncf::valid_max); 
      if (maxAtt != NULL) {
    ...
    case ncShort: {
      ...
      // cerr << "111111111 fieldName: " << _fhdr.field_name << endl;
      // cerr << "111111111 hasFillValue: " << hasFillValue << endl;
      // cerr << "111111111 hasValidMin: " << hasValidMin << endl;
      // cerr << "111111111 hasValidMax: " << hasValidMax << endl;
      // cerr << "111111111 fillValue: " << fillValue << endl;
      // cerr << "111111111 validMin: " << validMin << endl;
      // cerr << "111111111 validMax: " << validMax << endl;
