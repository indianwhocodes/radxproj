...
/////////////////////////////////////////////////////////////////////
//
// FieldData class holds Mdv field data as well as ptrs to a
// GridInfo object( access to 2D grid info and projection) ,
// VlevelInfo object (access to vertical level information)
//
//////////////////////////////////////////////////////////////
...
FieldData::FieldData(bool debug,
                     const MdvxField *mdvField,
                     const GridInfo *gridInfo,
                     const VlevelInfo *vlevelInfo,
...
int FieldData::addToNc(NcFile *ncFile, NcDim *timeDim,
...

{
  ...
  if (_ncfName.size() < 1) {
    errStr += "ERROR - FieldData::addToNc\n";
  ...
  if (_ncVar == NULL) {
    errStr += "WARNING - FieldData::addToNc\n";
    errStr += "  Cannot add variable to Nc file object\n";
  ...
  if (_packingUsed == DsMdvx::NCF_PACK_SHORT) {
    ...
    iret |= !_ncVar->add_att(Ncf::scale_factor, _scaleFactor);
    iret |= !_ncVar->add_att(Ncf::add_offset, _addOffset);
    iret |= !_ncVar->add_att(Ncf::valid_min, shortMin);
    iret |= !_ncVar->add_att(Ncf::valid_max, shortMax);
    iret |= !_ncVar->add_att(Ncf::FillValue, fillValue);
    ...
    iret |= !_ncVar->add_att(Ncf::scale_factor, _scaleFactor);
    iret |= !_ncVar->add_att(Ncf::add_offset, _addOffset);
    iret |= !_ncVar->add_att(Ncf::valid_min, (ncbyte) byteMin);
    iret |= !_ncVar->add_att(Ncf::valid_max, (ncbyte) byteMax);
    iret |= !_ncVar->add_att(Ncf::FillValue, (ncbyte) fillValue);
    ...
    iret |= !_ncVar->add_att(Ncf::valid_min, _minOut);
    iret |= !_ncVar->add_att(Ncf::valid_max, _maxOut);
    iret |= !_ncVar->add_att(Ncf::FillValue, _missing);

  }

  if ( _ncfStandardName.size() > 0) {
    iret |= !_ncVar->add_att(Ncf::standard_name,
                                _ncfStandardName.c_str());
  }
  
  iret |= !_ncVar->add_att(Ncf::long_name, _ncfLongName.c_str());
  iret |= !_ncVar->add_att(Ncf::units, _ncfUnits.c_str());
  ...
  if (_gridInfo->getProjType() != Mdvx::PROJ_LATLON ) {
    ...
    if (_outputLatlonArrays) {
      ...
      iret |= !_ncVar->add_att(Ncf::coordinates, auxVarNames);
    }

    if (_gridInfo->getNcProjVar() != NULL) {
      sprintf(auxVarNames, "%s", _gridInfo->getNcProjVar()->name());
      iret |= !_ncVar->add_att(Ncf::grid_mapping, auxVarNames);
    }
...
int FieldData::writeToFile(NcFile *ncFile, string &errStr)

{

  if (_ncVar == NULL) {
    errStr += "ERROR - FieldData::writeToFile\n";
...
int FieldData::_setCompression(NcFile *ncFile,
                               string &errStr)
  
{
  ...
  if (_debug) {
    cerr << "FieldData::_setCompression()" << endl;
  ...
  if (iret != NC_NOERR) {
    TaStr::AddStr(errStr, "ERROR: FieldData::_setCompression");
    TaStr::AddStr(errStr, "  Problem setting compression for var: ",
                  _ncVar->name());
    TaStr::AddStr(errStr, nc_strerror(iret));
    return -1;
  }
