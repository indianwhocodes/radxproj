...
#include <NcfMdv/Mdv2NcfTrans.hh>
...
///////////////////////////////////////////////////////////////////////////
// Parse Mdv data and record time information and create  unique FieldData,
// GridInfo, and VlevelInfo objects as necessary
//
...
// FieldData, GridInfo, and VlevelInfo  objects as necessary
//
// Returns 0 on success, -1 on failure
...

{
  ...
  // store in a GridInfo object. Vertical level data information is
  // stored in a VlevelInfo object.
  ...
  for (int i = 0; i < _mdv->getNFields(); i++) {    
    ...
    if (_leadTime != fhdr.forecast_delta) {
      TaStr::AddStr(_errStr, "ERROR - Mdv2NcfTrans::translate");
      TaStr::AddStr(_errStr,
                    "WARNING: Mdv2NcfTrans::readFile",
                    "  Forecast_delta differs from field data.");
      TaStr::AddStr(_errStr,
                    "  NetCDF file gets the lead time or forecast period "
                    " of the first field in the Mdv file");
    }
      
    // Create projection and grid information object

    GridInfo *gridInfo = new GridInfo(fhdr);
    ...
    // Create VlevelInfo object for vlevel_header_t struct
    ...
    VlevelInfo *vlevelInfo = new VlevelInfo(vlev, fhdr.vlevel_type, fhdr.nz);
    ...
    FieldData *fieldData = new FieldData(_debug, field, gridInfo, vlevelInfo,
...
int Mdv2NcfTrans::_addGlobalAttributes()
{
  ...
  if (_datasetInfo.size() > 0) {
    iret |= !_ncFile->add_att(Ncf::history,  _datasetInfo.c_str());
  }
  if (_mdv->_ncfInstitution.size() > 0) {
    iret |= !_ncFile->add_att(Ncf::institution ,
                              _mdv->_ncfInstitution.c_str());
  }
  if (_mdv->_ncfReferences.size() > 0) {
    iret |= !_ncFile->add_att(Ncf::references ,
                              _mdv->_ncfReferences.c_str());
  }

  // source

  iret |= !_ncFile->add_att(Ncf::source , _datasetSource.c_str());
  
  // title: use data set name

  iret |= !_ncFile->add_att(Ncf::title , _datasetName.c_str());

  // comment

  iret |= !_ncFile->add_att(Ncf::comment , _mdv->_ncfComment.c_str());
...
int Mdv2NcfTrans::_addDimensions()
{
  ...
  if (!(_timeDim = _ncFile->add_dim(Ncf::time, 1))) {
    return -1;
  }

  // add bounds dimension

  if (!(_boundsDim = _ncFile->add_dim(Ncf::bounds, 2))) {
    return -1;
  }
...
int Mdv2NcfTrans::_addTimeVariables()
{
  ...
  iret |= !_timeVar->add_att(Ncf::standard_name, Ncf::time);
  iret |= !_timeVar->add_att(Ncf::long_name, "Data time");
  iret |= !_timeVar->add_att(Ncf::units, Ncf::secs_since_jan1_1970);
  iret |= !_timeVar->add_att(Ncf::axis, "T");
  iret |= !_timeVar->add_att(Ncf::bounds, "time_bounds");

  DateTime timeCentroid(_timeCentroid);
  iret |= !_timeVar->add_att(Ncf::comment, timeCentroid.getW3cStr().c_str());
  ...
  if (_isForecast) {
    ...
    if ((_forecastRefTimeVar =
         _ncFile->add_var(Ncf::forecast_reference_time, ncDouble, _timeDim)) == NULL) {
      return -1;
    }
    iret |= !_forecastRefTimeVar->add_att(Ncf::standard_name,
                                          Ncf::forecast_reference_time);
    iret |= !_forecastRefTimeVar->add_att(Ncf::long_name, "Data generation time");
    iret |= !_forecastRefTimeVar->add_att(Ncf::units, Ncf::secs_since_jan1_1970);
    
    DateTime genTime(_timeGen);
    _forecastRefTimeVar->add_att(Ncf::comment, genTime.getW3cStr().c_str());
  
    // forecast period or lead time

    string forecastPeriodStr(Ncf::forecast_period);
    ...
    iret |= !_forecastPeriodVar->add_att(Ncf::standard_name, Ncf::forecast_period);
    iret |= !_forecastPeriodVar->add_att(Ncf::long_name, Ncf::forecast_period);
    iret |= !_forecastPeriodVar->add_att(Ncf::units, Ncf::seconds);
  ...
  if (_timeBegin != 0) {
    if ((_startTimeVar = _ncFile->add_var(Ncf::start_time,
                                          ncDouble, _timeDim)) == NULL) {
      return -1;
    }
    iret |= !_startTimeVar->add_att(Ncf::units, Ncf::secs_since_jan1_1970);
    iret |= !_startTimeVar->add_att(Ncf::comment, startTime.getW3cStr().c_str());
  }

  if (_timeEnd != 0) {
    if ((_stopTimeVar = _ncFile->add_var(Ncf::stop_time,
                                         ncDouble, _timeDim)) == NULL) {
      return -1;
    }
    iret |= !_stopTimeVar->add_att(Ncf::units, Ncf::secs_since_jan1_1970);
    iret |= !_stopTimeVar->add_att(Ncf::comment, stopTime.getW3cStr().c_str());
  }
  
  if (_timeBegin != _timeEnd &&
      _timeBegin <= _timeCentroid &&
      _timeEnd >= _timeCentroid) {
    if ((_timeBoundsVar = _ncFile->add_var(Ncf::time_bounds, ncDouble,
                                           _timeDim, _boundsDim)) == NULL) {
      return -1;
    }
    iret |= !_timeBoundsVar->add_att(Ncf::comment, "time_bounds also stored the start and stop times, provided the time variable value lies within the start_time to stop_time interval");
    iret |= !_timeBoundsVar->add_att(Ncf::units, Ncf::secs_since_jan1_1970);
  }
